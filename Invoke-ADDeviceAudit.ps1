Function Get-OldComputers {
    ...
    $searchBase = "OU=InformationSystems,OU=domain.Computers,DC=domain,DC=org"
    ...
    Function Disable-Computer {
        ...
        Move-ADObject -Identity $Computer.DistinguishedName -TargetPath "OU=Disabled,OU=domain.Computers,DC=domain,DC=org"
    ...
    Function Format-AuditReport {
        ...
        $WorkBookPath = "H:\Profile\Documents\$($Env:USERNAME)\My Documents\DeviceAuditReport_$currentDate.xlsx"
    ...
    $Exclusions = @("HVR001L66", "HVR082L13")

     foreach ($computer in $computers) { 
        if($Exclusions -contains $computer.Name) {
            Write-Host "$($computer.Name) has been excluded from audit" -ForegroundColor Yellow
            continue
        }
